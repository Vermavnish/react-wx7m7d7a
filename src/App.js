// Bartan Booking App - React + Tailwind CSS (Deployable on GitHub + Netlify) import React, { useState } from "react"; import { format } from "date-fns";
...
export default function App() { const [selectedDate, setSelectedDate] = useState("2025-05-06"); const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [formItems, setFormItems] = useState({ ...initialInventory }); const [message, setMessage] = useState("");

const handleSubmit = () => { const alreadyBooked = bookings.find((b) => b.date === selectedDate); if (alreadyBooked) { setMessage("Is tareekh par bartan pehle se book hain."); return; }
...
return ( <div className="max-w-2xl mx-auto p-4 font-sans"> <h1 className="text-3xl font-bold mb-4 text-center">Bartan Booking App</h1>
